...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const [pay] = await rest(`payments?id=eq.${payment_id}&select=user_id,valor,quota_id,motivo_rejeicao`);
    ...
    const link = pay.quota_id ? `${origin}/app/pagamento/${pay.quota_id}` : `${origin}/app/quotas`;
    const firstName = profile.nome.split(" ")[0];
    const quotaLabel = quotaNumero != null ? `Quota #${quotaNumero}` : "sua quota";
    const motivo = pay.motivo_rejeicao ? `<p style="font-size:14px;padding:12px;background:#fef2f2;border-radius:8px"><strong>Motivo:</strong> ${pay.motivo_rejeicao}</p>` : "";

    const html = `
      <div style="font-family:-apple-system,sans-serif;max-width:560px;margin:0 auto;padding:24px;color:#1a1a1a">
        <h1 style="font-size:22px;margin:0 0 12px">Ô ${firstName}, deu ruim no comprovante</h1>
        <p style="font-size:15px;line-height:1.5">O comprovante da <strong>${quotaLabel}</strong> precisou ser rejeitado. Calma — é só reenviar e seguir o jogo.</p>
        ${motivo}
        <p style="margin:24px 0"><a href="${link}" style="background:#dc2626;color:#fff;padding:12px 24px;border-radius:999px;text-decoration:none;font-weight:bold">Reenviar comprovante</a></p>
      </div>`;
    const text = `Ô ${firstName}, o comprovante da ${quotaLabel} foi rejeitado.${pay.motivo_rejeicao ? " Motivo: " + pay.motivo_rejeicao : ""}\n\nReenvia aqui: ${link}`;

    const sendRes = await fetch(RESEND_API_URL, {
      method: "POST",
      headers: { "Content-Type": "application/json", Authorization: `Bearer ${RESEND_API_KEY}` },
      body: JSON.stringify({ from: FROM, to: [profile.email], subject: `Comprovante da ${quotaLabel} precisou ser rejeitado`, html, text }),
    });
