...
// Chamada pela função SQL alertar_capacidade() com header x-cron-secret.
...
// Fix Alerta #1 — escapa qualquer string antes de injetar no template HTML.
...
function fmtMetrica(key: string, m: Metrica): string {
  const pct = Math.round(Number(m.pct) || 0);
  if (key === "storage") return `${Number(m.atual_mb) || 0} MB / ${Number(m.max_mb) || 0} MB (${pct}%)`;
  return `${Number(m.atual) || 0} / ${Number(m.max) || 0} (${pct}%)`;
}
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const nivelRaw: string = check?.nivel_geral ?? "verde";
    const nivel = ["verde", "amarelo", "vermelho"].includes(nivelRaw) ? nivelRaw : "verde";
    ...
    const linhas = Object.entries(metricas)
      .map(([k, v]) => {
        ...
        const corKey = ["verde", "amarelo", "vermelho"].includes(m?.cor) ? m.cor : "verde";
        const cor = CORES_HEX[corKey];
        const destaque = corKey !== "verde";
        const label = escapeHtml(NOMES[k] ?? k);
        return `<li style="margin:6px 0;color:${cor};${destaque ? "font-weight:bold" : ""}">${label}: ${escapeHtml(fmtMetrica(k, m))} — ${escapeHtml(corKey.toUpperCase())}</li>`;
      })
      .join("");

    const corNivel = CORES_HEX[nivel];
    const verificadoEm = escapeHtml(check?.verificado_em ?? new Date().toISOString());
    const html = `
      <div style="font-family:-apple-system,sans-serif;max-width:560px;margin:0 auto;padding:24px;color:#1a1a1a">
        <h1 style="font-size:20px;margin:0 0 12px">⚠️ Alerta de capacidade — <span style="color:${corNivel}">${escapeHtml(nivel.toUpperCase())}</span></h1>
        ...
        <p style="font-size:12px;color:#666;margin-top:16px">Verificado em ${verificadoEm}</p>
      </div>`;
    ...
    if (!sendRes.ok) {
      ...
      return json({ error: `Resend ${sendRes.status}` }, 500);
    }
    const sent = (await sendRes.json()) as { id: string };
    // Não vazar lista de emails na resposta — apenas a contagem.
    return json({ success: true, email_id: sent.id, destinatarios_count: destinatarios.length });
  } catch (e) {
    console.error(e);
