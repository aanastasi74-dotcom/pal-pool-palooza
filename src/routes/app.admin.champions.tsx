  ...
  return (
    <div className="space-y-5">
      ...
      <section className="rounded-2xl border border-primary/40 bg-primary/5 p-4 shadow-card">
        <p className="text-xs uppercase tracking-widest text-muted-foreground">Quotas manifestadas</p>
        <p className="mt-1 font-display text-4xl font-extrabold">
          {quotasInternas} <span className="text-base font-medium text-muted-foreground">/ {total?.quorum ?? 35}</span>
        </p>
      </section>
      ...
      <LotesAprovacaoSection slug="champions2627" />
...
type FilmeStatus = {
  ...
  termometro: { quotas: number; quorum: number };
  youtube: string;
};

function EmailFilmeCard() {
  const qc = useQueryClient();
  const [confirmEnviar, setConfirmEnviar] = useState(false);
  const [confirmForce, setConfirmForce] = useState(false);

  const status = useQuery({
    queryKey: ["champions", "admin", "filme"],
    queryFn: async (): Promise<FilmeStatus> => {
      const { data, error } = await supabase.functions.invoke("enviar-email-filme", {
        method: "GET",
      });
      if (error) throw error;
      if ((data as any)?.error) throw new Error((data as any).error);
      return data as FilmeStatus;
    },
  });
  ...
  const disparo = useMutation({
    ...
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["champions", "admin", "filme"] });
    },
  });
  ...
  const enviar = async (force: boolean) => {
    try {
      ...
      toast.error(e?.message ?? "Falha ao disparar o email do filme.");
    }
  };
  ...
  return (
    <section className="rounded-2xl border border-border bg-card p-5 shadow-card">
      <h2 className="flex items-center gap-2 font-display text-lg font-bold">
        <Film className="h-4 w-4 text-primary" /> E-mail do filme (reforço)
      </h2>
      {status.isLoading ? (
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          Destinatários: <strong>{destinatarios}</strong> · Termômetro:{" "}
          <strong>
            {status.data?.termometro?.quotas ?? 0}/{status.data?.termometro?.quorum ?? 0} quotas
          </strong>
          <br />
          Status:{" "}
          {enviadoEm ? (
      ...
      <ConfirmDialog
        ...
        title="Disparar e-mail do filme?"
        description={`Isso enviará o e-mail para ${destinatarios} cadastrados. Confirmar?`}
        confirmLabel="Disparar"
        destructive={false}
        onConfirm={() => {
      ...
      <ConfirmDialog
        ...
        description={`O e-mail já foi enviado antes. Reenviar para ${destinatarios} cadastrados mesmo assim?`}
